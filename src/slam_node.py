...
class PoseBasedSlam(object):
    ...
    def __init__(self) -> None:
        ...
        # self.ICP_Rk = np.diag([0.05,0.05,1.0])
                ...
                if len(self.map) >= 1:
                    ...
                    for mapped_scan_index, mapped_scan in enumerate(self.map[:-1]):
                        ...
                        if self.do_scans_overlap(new_scan, mapped_scan) == True:
                            ...
                            if len(self.map) - mapped_scan_index > 6:
                                continue
    ...
    def lidar_update(self, scan_index, measurement, Rk):
        ...
        if np.sqrt(D) <= self.chi_square_min_dist_thresh:
            ...
            all_aligned_scans = np.zeros([1, 3])
            
            #transform all the scans in the map to the world frame using the robot's state vector
            for i in range(len(self.map)):

                scan = self.map[i]
                scan_pose = self.xk_slam[3*i:3*i+3, :]
                
                #transform the scan to the world frame and append it to the all_aligned_scans array
                all_aligned_scans = np.vstack((all_aligned_scans, self.transform_pcd(scan, scan_pose)))
            
            #publish all aligned scans
            self.publish_all_aligned_scans(all_aligned_scans)
