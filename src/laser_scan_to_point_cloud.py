...
class LaserProjection:
    ...
    def __projectLaser(self, scan_in, range_cutoff, channel_options):
        ...
        # Remove all ranges that are outside of the scanner specified
        # bounds.
        # mask = np.isfinite(ranges)
        # filtered_ranges = ranges[mask]
        # output = filtered_ranges * self.__cos_sin_map[mask]
        ranges = np.where(np.logical_and(ranges >= scan_in.range_min,
                                            ranges <= scan_in.range_max),
                            ranges, self.LASER_SCAN_INVALID)
